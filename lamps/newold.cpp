...
int recur(int n, int val){
	if(n<=0){
		...
		ans.insert(recur(n-1, button1(val)));
		ans.insert(recur(n-1, button2(val)));
		ans.insert(recur(n-1, button3(val)));
		ans.insert(recur(n-1, button4(val)));
		return 0;
	}
}
...
int main(){
	...
	ans.insert(recur(operations % 6, init));
	bool flag = true;
